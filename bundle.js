...
/***/ function(module, exports, __webpack_require__) {
	...
	(function () {
	  const data = {};
	  const Sankey = __webpack_require__(7).sankey;
	  const dataJson = __webpack_require__(11);
	  const $ = __webpack_require__(12);
	  const nodeMap = {};
	  const margin = { top: 30, right: 1, bottom: 50, left: 1 };
	  const width = 960 - margin.left - margin.right;
	  const height = 2150 - margin.top - margin.bottom;
	  ...
	  var button = $('#rotateButton').on('click', function (e) {
	    console.log($("#goc").hasClass("rotate_false"));
	    if ($("#goc").hasClass("rotate_false")) {
	      $("#goc").removeClass("rotate_false");
	      $("#goc").addClass("rotate_true");
	    } else {
	      $("#goc").removeClass("rotate_true");
	      $("#goc").addClass("rotate_false");
	    }
	  });
	  $("#rangeInput").prop({
	    max: years.length - 1
	  ...
	  function update(year) {
	    ...
	    const svg = d3.select("#goc").append("svg").attr({
	      width: width + margin.left + margin.right,
	      height: height + margin.top + margin.bottom
	    }).append("g").attr("transform", "translate(" + margin.left + "," + margin.top + ")");

	    const sankey = Sankey().nodeWidth(30).nodePadding(10).size([width, height]);

	    const path = sankey.link();

	    sankey.nodes(data.nodes).links(data.links).layout(32);

	    const link = svg.append("g").selectAll(".link").data(data.links).enter().append("path").attr('class', function (d) {
	      return d.class;
